                ...
                <div class="modal-body">
                    <form>
                        <div class="form-group">
                            <input type="hidden" class="form-control" id="id" name="id" value="{{$data->list->id }}">
                            <label for="name" class="control-label">Nama <span id="alert" class="text-danger"></span></label>
                            <input type="text" onchange="cek(this.value)" class="form-control" required id="name" name="name" value="{{$data->list->name}}">
                            </div>
                            <div class="form-group">
                            <label for="email" class="control-label">Email</label> 
                            <input type="text" class="form-control" required id="email" name="email" value="{{$data->list->email}}">
                            </div>
                            <div class="form-group">
                            ...
                            @if(Auth::user()->role_id == '1')
                                <option value="Admin Gudang" @if($data->list->role_id=='1') selected :'disabled' @endif>Admin Gudang</option>@endif
                                @if(Auth::user()->role_id == '2')
                                <option value="Admin Penjualan"  @if($data->list->role_id=='2') selected : 'disabled' @endif>Admin Penjualan</option>@endif
                                @if(Auth::user()->role_id == '3')
                                <option value="Pemilik" @if($data->list->role_id=='3') selected @endif>Pemilik</option>@endif
                                </select>
                            </div>
                    </form>
                </div>
